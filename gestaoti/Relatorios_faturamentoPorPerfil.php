...
if($flag == "1"){
	if($_SESSION["SEQ_PERFIL_ACESSO"] == "5" || $_SESSION["SEQ_PERFIL_ACESSO"] == "2"){ // ==== Gestor PRTI pode ver tudo
		...
		if($v_SEQ_EQUIPE_TI == ""){
			$pagina->ScriptAlert("Acesso permitido apenas aos dados de sua equipe.");
			$flag="";
		}
	}else{ // Colaborador ve somente o seu
		if($v_SEQ_EQUIPE_TI == ""){
			$pagina->ScriptAlert("Acesso permitido apenas aos seus dados.");
			$flag="";
		}
...
if($flag == ""){
	...
	<script language="javascript">
		...
		function do_CarregarComboProfissional() {
			x_CarregarComboProfissional(document.form.v_SEQ_PERFIL_RECURSO_TI.value, retorno_CarregarComboProfissional);
		}
		...
		function retorno_CarregarComboProfissional(val) {
			if(val == ""){
				fEncheComboBoxPlus("", document.form.v_NUM_MATRICULA_RECURSO, "Prossifionais - Selecione o cargo");
			}else{
		...
		function fValidaFormLocal(){
			...
			document.form.action = 'Relatorios_faturamentoPorPerfil.php';
		...
		function fValidaFormLocal1(){
				...
				document.form.action = 'Relatorios_faturamentoPorPerfilPDF.php';
	...
	if($pagina->isGestorTI($_SESSION["SEQ_PERFIL_ACESSO"]) || $pagina->isAdministrador($_SESSION["SEQ_PERFIL_ACESSO"])){
		...
		if( $v_NOM_PERFIL_RECURSO_TI == ""){
			...
			//perfil_recurso_ti
			$aItemOptionPerfilRecursoTi = Array();
			$aItemOptionPerfilRecursoTi[] = array("", "", "Profissionais - Selecione o cargo");
			$vItemTodosPerfilRecursoTi = "N";
		...
		if($v_SEQ_EQUIPE_TI != ""){
			...
			$aItemOptionProfissional[] = array("", "", "Prossifionais - Selecione o cargo");
		...
		$aItemOptionEquipe = $equipe_ti->combo("NOM_EQUIPE_TI", $pagina->iif($v_SEQ_EQUIPE_TI=="", $_SESSION["SEQ_EQUIPE_T		I"], $v_SEQ_EQUIPE_TI));
		$vItemTodosEquipe = "N";*/
		...
		$aItemOptionPerfilRecursoTi = $perfil_recurso_ti->combo("NOM_PERFIL_RECURSO_TI", $pagina->iif($v_SEQ_PERFIL_RECURSO_TI=="", $_SESSION["SEQ_PERFIL_RECURSO_TI"], $v_SEQ_PERFIL_RECURSO_TI ));
		$vItemTodosPerfilRecursoTi = "N";
		...
		$pagina->CampoSelect("v_NOM_PERFIL_RECURSO_TI", "N", "Profissional", $vItemTodosProfissional, $aItemOptionProfissional, "Escolha", "", "combo_profissional"), "left", "id=".$pagina->GetIdTable());		*/

		//perfil_recurso_ti
		$pagina->LinhaCampoFormulario("Executor:", "right", "N",
		$pagina->CampoSelect("v_SEQ_PERFIL_RECURSO_TI", "N", "Profissional", $vItemTodosPerfilRecursoTi, $aItemOptionPerfilRecursoTi, "Profissional - Selecione o cargo", "do_CarregarComboProfissional()", "combo_profissional"), "left", "id=".$pagina->GetIdTable());
	...
	if($equipe_ti->database->rows > 0){
		while ($row = oci_fetch_array($equipe_ti->database->result, OCI_BOTH)){
...
//***
			...
			if($recurso_ti->database->rows == 0){
				...
				while ($row1 = oci_fetch_array($recurso_ti->database->result, OCI_BOTH)){
					...
					if($time_sheet->database->rows == 0){
						...
						while ($row2 = oci_fetch_array($time_sheet->database->result, OCI_BOTH)){
