...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Alteração de Equipe de Atendimento"); // Indica o título do cabeçalho da página
	// Itens das abas
	$aItemAba = Array( array("Equipe_tiPesquisa.php", "", "Pesquisa"),
						array("Equipe_tiCadastro.php", "", "Adicionar"),
		 			    array("Equipe_tiAlteracao.php", "tabact", "Alterar") );
	$pagina->SetaItemAba($aItemAba);
	// Pesquisar
	$banco->select($v_SEQ_EQUIPE_TI);

	// Inicio do formulário
	$pagina->MontaCabecalho();
	print $pagina->CampoHidden("v_SEQ_EQUIPE_TI", $v_SEQ_EQUIPE_TI);
	print $pagina->CampoHidden("flag", "1");
	$pagina->AbreTabelaPadrao("center", "95%");
	$pagina->LinhaCampoFormulario("Nome:", "right", "S", $pagina->CampoTexto("v_NOM_EQUIPE_TI", "S", "Nome", "60", "120", "$banco->NOM_EQUIPE_TI"), "left");
	$pagina->LinhaCampoFormulario("Matricula do Líder:", "right", "S",
								  $pagina->CampoTexto("v_NUM_MATRICULA_LIDER", "S", "Matrícula do Líder" , "11", "11", $empregados->GetNomLoginRedeMatricula($banco->NUM_MATRICULA_LIDER), "").
								  $pagina->ButtonProcuraEmpregado("v_NUM_MATRICULA_LIDER", "TI")
								  , "left");

	$pagina->LinhaCampoFormulario("Matricula do Substituto:", "right", "S",
								  $pagina->CampoTexto("v_NUM_MATRICULA_SUBSTITUTO", "S", "Matrícula do Substituto" , "11", "11", $empregados->GetNomLoginRedeMatricula($banco->NUM_MATRICULA_SUBSTITUTO), "").
								  $pagina->ButtonProcuraEmpregado("v_NUM_MATRICULA_SUBSTITUTO", "TI")
								  , "left");

	$pagina->LinhaCampoFormulario("Matricula do Priorizador:", "right", "N",
								  $pagina->CampoTexto("v_NUM_MATRICULA_PRIORIZADOR", "N", "Matrícula do Priorizador" , "11", "11", $empregados->GetNomLoginRedeMatricula($banco->NUM_MATRICULA_PRIORIZADOR), "").
	...
	$banco->update($v_SEQ_EQUIPE_TI);
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Equipe_tiPesquisa.php?v_SEQ_EQUIPE_TI=$v_SEQ_EQUIPE_TI");
	}
}
?>
