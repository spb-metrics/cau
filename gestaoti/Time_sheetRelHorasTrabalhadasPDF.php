...
// ====================================================================================================================================
	...
	$pdf->titulo = "Relatório de Horas Trabalhadas por Dia";
	...
	if($equipe_ti->database->rows > 0){
		while ($row = pg_fetch_array($equipe_ti->database->result)){
			...
			$pdf->Cell(190, 5, $row["NOM_EQUIPE_TI"], 0, 0, "left", 1);
			...
			if($recurso_ti->database->rows == 0){
				...
				while ($row1 = pg_fetch_array($recurso_ti->database->result)){
					...
					if($time_sheet->database->rows == 0){
						...
						$pdf->Cell(140, 5, "Nenhum lançamento registrado no período", 0, 0, "C", 1);
					}else{
						...
						while ($row2 = pg_fetch_array($time_sheet->database->result)){
							...
							$header = array();
							$header[] = array($row2["DTH_INICIO"], "C", "", "");
						...
						$pdf->Cell(0, 5, "", 0, 1);
						$pdf->Cell(23, 5, "", 0, 0, "C", 0);
						$pdf->Cell(46, 5, "Data", 0, 0, "C", 1);
						$pdf->Cell(46, 5, "Tempo Registrado", 0, 0, "C", 1);
						$pdf->Cell(46, 5, "Qtd. Lançamentos", 0, 0, "C", 1);

						for($i=0; $i<count($tabela); $i++){
							if($i % 2 == 0) $pdf->SetFillColor(255,255,255);
							else $pdf->SetFillColor(224,224,224);
							$pdf->Cell(0, 5, "", 0, 1);
							$pdf->Cell(23, 5, "", 0, 0, "center", 0);
							for($j=0; $j<count($tabela[$i]); $j++){
								$pdf->Cell(46, 5, $tabela[$i][$j][0], 0, 0, $tabela[$i][$j][1], 1);
							}
						}
						$pdf->SetFillColor(202,202,202);
						$pdf->Cell(0, 5, "", 0, 1);
						$pdf->Cell(23, 5, "", 0, 0, "C");
						$pdf->Cell(46, 5, "Total", 0, 0, "C", 1);
						$pdf->Cell(46, 5, $pagina->secondsToHours($v_SOMA, 1), 0, 0, "L", 1);
						$pdf->Cell(46, 5, $v_SOMA_LANCAMENTOS, 0, 0, "R", 1);
