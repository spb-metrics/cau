...
if($v_SEQ_RDM != "" && $v_ACAO != ""){
	...
	if($v_ACAO=="FINALIZAR"){		 
		$situacaoRDM->setSITUACAO($situacaoRDM->FINALIZADA_COM_ROLL_BACK);
	}else if($v_ACAO=="ERRO"){
		$situacaoRDM->setSITUACAO($situacaoRDM->FINALIZADA_COM_ERRO);
	} 
...
}else if($v_SEQ_ID_ATM != "" && $v_ACAO_ATM != ""){	
$RDM = new rdm();
$situacaoRDM = new situacao_rdm();		 
...
$pagina->SettituloCabecalho("Execução de Rollback da RDM"); // Indica o título do cabeçalho da página
...
if(($RDM->getSITUACAO_ATUAL()==$situacaoRDM->EXECUTANDO_ROLL_BACK||
	...
	($pagina->isAdministrador($_SESSION["SEQ_PERFIL_ACESSO"]) ||
	 $pagina->isExecutorDeMudancas($_SESSION["SEQ_PERFIL_ACESSO"]))){			
		//$aItemAba[] = array("RDMExecutarRollBack.php?v_SEQ_RDM=".$v_SEQ_RDM, "", "Validar");
		...
		$SituacaoAtividadesAnterior ="";
		 ...
		 while ($row = pg_fetch_array($atividadeRBRDM->database->result)){
			$valor ="";
		 	...
		 	if(($row["situacao"] != $atividadeRBRDM->FINALIZADA)&&
		 		($row["situacao"] != $atividadeRBRDM->FALHA_NA_EXECUCAO)&&
...
//		 		   $SituacaoAtividadesAnterior == ""){
		...
		$pagina->LinhaCampoFormularioColspanDestaqueCenter("&nbsp;", 5);	
		...
		$valor .= $pagina->CampoButton("if(fValidaForm()){if(confirm('A RDM terá sua situação alterado para finalizada com erro.  Confirma ?')){document.form.v_ACAO.value='ERRO';return true;}else{return false;}}else{return false;}", " Finalizar com Erro ");
		$valor .=" &nbsp;&nbsp;&nbsp;&nbsp;".$pagina->CampoButton("if(confirm('Confirma finalização a RDM?')){document.form.v_ACAO.value='FINALIZAR';return true;}else{return false;}", " Finalizar com Rollback ");
	
		//$valor .="&nbsp;&nbsp;<input type=\"button\" id=\"campo_texto\"  style=\"cursor:hand;width:12%;\"  value=\"Finalizar com Erro\" alt=\"Finalizada com Erro\" border=\"0\" onClick=\"alert('Não existem atividades habilitadas');\" />";
		//$valor .="&nbsp;&nbsp;<input type=\"button\" id=\"campo_texto\"  style=\"cursor:hand;width:12%;\"  value=\"Finalizar com Erro\" alt=\"Finalizada com Rollback\" border=\"0\" onClick=\"alert('Não existem atividades habilitadas');\" />";
