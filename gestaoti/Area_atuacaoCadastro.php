...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Cadastro de Área de Atuação"); // Indica o título do cabeçalho da página
	// Itens das abas
	...
	$pagina->LinhaCampoFormulario("Nome:", "right", "S", $pagina->CampoTexto("v_NOM_AREA_ATUACAO", "S", "Nome", "60", "60", ""), "left"); 
	...
	$banco->insert();
	// Código inserido: $banco->SEQ_AREA_ATUACAO
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Area_atuacaoPesquisa.php?v_SEQ_AREA_ATUACAO=$banco->SEQ_AREA_ATUACAO");
	}
}
?>
