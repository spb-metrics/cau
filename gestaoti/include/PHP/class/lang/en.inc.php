...
// Created: 	2006-01-25
// Ver:		$Id: en.inc.php 899 2007-05-08 03:34:30Z ljp $
//
...
HTTP headers have already been sent.<br>Caused by output FROM gestaoti.file <b>%s</b> at line <b>%d</b>.</td></tr><tr><td><b>Explanation:</b><br>HTTP headers have already been sent back to the browser indicating the data as text before the library got a chance to send it\'s image HTTP header to this browser. This makes it impossible for the library to send back image data to the browser (since that would be interpretated as text by the browser and show up as junk text).<p>Most likely you have some text in your script before the call to <i>Graph::Stroke()</i>. If this texts gets sent back to the browser the browser will assume that all data is plain text. Look for any text, even spaces and newlines, that might have been sent back to the browser. <p>For example it is a common mistake to leave a blank line before the opening "<b>&lt;?php</b>".</td></tr></table>',2),
...
2007 => array('Cannot create GroupBarPlot FROM gestaoti.empty plot array.',0),
2008 => array('Group bar plot element nbr %d is undefined or empty.',0),
2009 => array('One of the objects submitted to GroupBar is not a BarPlot. Make sure that you create the GroupBar plot FROM gestaoti.an array of BarPlot or AccBarPlot objects. (Class = %s)',1),
2010 => array('Cannot create AccBarPlot FROM gestaoti.empty plot array.',0),
2011 => array('Acc bar plot element nbr %d is undefined or empty.',1),
2012 => array('One of the objects submitted to AccBar is not a BarPlot. Make sure that you create the AccBar plot FROM gestaoti.an array of BarPlot objects. (Class=%s)',1),
...
6008 => array('You have specified a constrain FROM gestaoti.row=%d to row=%d which does not have any activity',2),
6009 => array('Unknown constrain type specified FROM gestaoti.row=%d to row=%d',2),
...
19002 => array('Invalid input data for spline. Two or more consecutive input X-values are equal. Each input X-value must differ since FROM gestaoti.a mathematical point of view it must be a one-to-one mapping, i.e. each X-value must correspond to exactly one Y-value.',0),
...
25005 => array('Can\'t access PHP_SELF, PHP global variable. You can\'t run PHP FROM gestaoti.command line if you want to use the \'auto\' naming of cache or image files.',0),
...
25045 => array('Font families FF_HANDWRT and FF_BOOK are no longer available due to copyright problem with these fonts. Fonts can no longer be distributed with JpGraph. Please download fonts FROM gestaoti.http://corefonts.sourceforge.net/',0),
...
25085 => array('An image can not be created FROM gestaoti.the supplied string. It is either in a format not supported or the string is representing an corrupt image.',0),
...
25116 => array('Cant open file FROM gestaoti.cache "%s"',1), 
...
25124 => array('The input data array must have consecutive values FROM gestaoti.position 0 and forward. The given y-array starts with empty values (NULL)',0),
