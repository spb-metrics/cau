...
class anexo_rdm{
	...
	// ***********************
	var $SEQ_ANEXO_RDM;   // KEY ATTR. WITH AUTOINCREMENT
	...
	function getSEQ_ANEXO_RDM(){
		return $this->SEQ_ANEXO_RDM;
	}
	...
	function setSEQ_ANEXO_RDM($val){
		$this->SEQ_ANEXO_RDM =  $val;
	}
	...
	// **********************
	function select($id){
		$sql = "SELECT SEQ_ANEXO_RDM , SEQ_RDM , NOM_ARQUIVO_SISTEMA , NOM_ARQUIVO_ORIGINAL , to_char(DTH_ANEXO, 'dd/mm/yyyy') as DTH_ANEXO, DTH_ANEXO as DTH_ANEXO_DATA , NUM_MATRICULA
			    FROM gestaoti.anexo_rdm
				WHERE SEQ_ANEXO_RDM = $id";
		$result = $this->database->query($sql);
		$result = $this->database->result;
		$row = pg_fetch_object($result, 0);
		$this->SEQ_ANEXO_RDM = $row->seq_anexo_rdm;
	...
	function selectParam($orderBy = 1, $vNumPagina = "", $vQtdRegistros = "20"){
		...
		$sqlSelect = "SELECT SEQ_ANEXO_RDM , SEQ_RDM , NOM_ARQUIVO_SISTEMA , NOM_ARQUIVO_ORIGINAL ,
							 to_char(DTH_ANEXO, 'dd/mm/yyyy hh24:mi.ss') as DTH_ANEXO, DTH_ANEXO as DTH_ANEXO_DATA, NUM_MATRICULA,
							 b.NOM_COLABORADOR ";
		$sqlCorpo  = "FROM gestaoti.anexo_rdm a, gestaoti.viw_colaborador b
					  WHERE a.NUM_MATRICULA = b.NUM_MATRICULA_COLABORADOR ";

		if($this->SEQ_ANEXO_RDM != ""){
			$sqlCorpo .= "  and SEQ_ANEXO_RDM = $this->SEQ_ANEXO_RDM";
		}
		if($this->SEQ_RDM != ""){
	...
	// **********************
	function delete($id){
		$sql = "DELETE FROM gestaoti.anexo_rdm WHERE SEQ_ANEXO_RDM = $id";
		$result = $this->database->query($sql);
	}
	...
	function insert(){
		$this->SEQ_ANEXO_RDM = $this->database->GetSequenceValue("gestaoti.SEQ_ANEXO_RDM");
		$this->NOM_ARQUIVO_SISTEMA = $this->iif($this->EXTENCAO_ARQUIVO=="", "", $this->SEQ_ANEXO_RDM.".".$this->EXTENCAO_ARQUIVO);

		$sql = "INSERT INTO gestaoti.anexo_rdm(SEQ_ANEXO_RDM,
										  SEQ_RDM,
										  NOM_ARQUIVO_SISTEMA,
							 ...
							 VALUES (".$this->iif($this->SEQ_ANEXO_RDM=="", "NULL", "'".$this->SEQ_ANEXO_RDM."'").",
