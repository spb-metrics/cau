...
if($flag == ""){
        $banco->select($v_SEQ_UNIDADE_ORGANIZACIONAL);
	...
	$aItemAba = Array( array("Unidade_organizacionalPesquisa.php", "", "Pesquisa"),
			   array("Unidade_organizacionalCadastro.php", "tabact", "Adicionar"),
                           array("Unidade_organizacionalAlteracao.php", "tabact", "Alterar") );
	...
	print $pagina->CampoHidden("flag", "1");
        ...
        print $pagina->CampoHidden("v_SEQ_UNIDADE_ORGANIZACIONAL", $v_SEQ_UNIDADE_ORGANIZACIONAL);
	$pagina->AbreTabelaPadrao("center", "85%");
        
        $pagina->LinhaCampoFormulario("Nome:", "right", "S", $pagina->CampoTexto("v_NOM_UNIDADE_ORGANIZACIONAL", "S", "Nome", "60", "60", $banco->NOM_UNIDADE_ORGANIZACIONAL), "left");
        $pagina->LinhaCampoFormulario("Sigla:", "right", "S", $pagina->CampoTexto("v_SGL_UNIDADE_ORGANIZACIONAL", "S", "Nome", "60", "60", $banco->SGL_UNIDADE_ORGANIZACIONAL), "left");
	// Montar a combo da tabela edificacao
	require_once 'include/PHP/class/class.edificacao.php';
	$edificacao = new edificacao();
	$pagina->LinhaCampoFormulario("Unidade organizacional superior:", "right", "N", $pagina->CampoSelect("v_SEQ_UNIDADE_ORGANIZACIONAL_PAI", "S", "", "S", $unidade_organizacional->combo("NOM_UNIDADE_ORGANIZACIONAL", $banco->SEQ_UNIDADE_ORGANIZACIONAL_PAI)), "left");
	...
	// AREA ENVOLVIDA ===================================================================================================
		...
		function fAdicionaResponsavel(v_SEQ_PESSOA, v_NOME){
			if(v_SEQ_PESSOA != ""){
				if(InserirItemArray(aResponsavelAux, v_SEQ_PESSOA) == true){
					InserirItemArray(aResponsavel, v_SEQ_PESSOA);
					...
					setRowIndex(aResponsavelAux, v_SEQ_PESSOA, linha.rowIndex);
					setRowIndex(aResponsavel, v_SEQ_PESSOA, linha.rowIndex);
	...
	$header[] = array($pagina->CampoSelect("v_SEQ_PESSOA", "S", "", "S", $empregados->combo("NOME", ""))."&nbsp;".
                          $pagina->CampoButton("return fAdicionaResponsavel(document.form.v_SEQ_PESSOA.value, v_SEQ_PESSOA.options[v_SEQ_PESSOA.selectedIndex].text);", "Adicionar", "button")
                            , "center", "", "");
	...
	$pagina->LinhaCampoFormularioColspan("center", "&nbsp;", 2);
        ...
        // Inserir os responsáveis já existentes
        ?>
        <script language="javascript">
            <?
            // Buscar informações sobre o responsável
            require_once 'include/PHP/class/class.responsavel_unidade_organizacional.php';
            $responsavel_unidade_organizacional = new responsavel_unidade_organizacional();
            $responsavel_unidade_organizacional->setSEQ_UNIDADE_ORGANIZACIONAL($v_SEQ_UNIDADE_ORGANIZACIONAL);
            ...
            if($responsavel_unidade_organizacional->database->rows > 0){
                while ($rowUnidadeOrganizacional = pg_fetch_array($responsavel_unidade_organizacional->database->result)){
                    ...
                    ?>
                    fAdicionaResponsavel("<?=$rowUnidadeOrganizacional["seq_pessoa"]?>", "<?=$rowUnidadeOrganizacional["nome"]?>");
                    <?
                }
            }
	...
	$banco->setNOM_UNIDADE_ORGANIZACIONAL($v_NOM_UNIDADE_ORGANIZACIONAL);
        ...
        $banco->update($v_SEQ_UNIDADE_ORGANIZACIONAL);
