...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Cadastro de Status dos Sistemas de Informação"); // Indica o título do cabeçalho da página
	// Itens das abas
	...
	$pagina->LinhaCampoFormulario("Nome:", "right", "S", $pagina->CampoTexto("v_NOM_STATUS_SOFTWARE", "S", "Nome", "60", "60", ""), "left"); 
	...
	$banco->insert();
	// Código inserido: $banco->SEQ_STATUS_SOFTWARE
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Status_softwarePesquisa.php?v_SEQ_STATUS_SOFTWARE=$banco->SEQ_STATUS_SOFTWARE");
	}
}
?>
