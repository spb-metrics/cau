...
$banco_perfil_recurso_ti = new perfil_recurso_ti();
	...
	// Itens das abas
	$aItemAba = Array( array("Hist_Time_Sheet.php", "", "Pesquisa"),
					   array("#", "tabact", "Detalhes")
						 );
	$pagina->SetaItemAba($aItemAba);
	// pesquisa
	$banco_timesheet->select($v_SEQ_TIME_SHEET);
	
	// Inicio do formulário
	$pagina->MontaCabecalho();
	print $pagina->CampoHidden("flag", "1");
	print $pagina->CampoHidden("v_SEQ_TIME_SHEET", $banco->SEQ_TIME_SHEET);
	...
	$banco_tarefa->select($banco_timesheet->SEQ_TAREFA_TI);
	...
	$dataInicio = substr($banco_timesheet->DAT_INICIO,8,2)."/".substr($banco_timesheet->DAT_INICIO,5,2)."/".substr($banco_timesheet->DAT_INICIO,0,4);
	$pagina->LinhaCampoFormulario("Data Início:", "right", "N", $dataInicio, "left", "id=".$pagina->GetIdTable(), "30%", "70%");		
	$dataFim = substr($banco_timesheet->DAT_FIM,8,2)."/".substr($banco_timesheet->DAT_FIM,5,2)."/".substr($banco_timesheet->DAT_FIM,0,4);
	$pagina->LinhaCampoFormulario("Data Fim:", "right", "N", $dataFim, "left", "id=".$pagina->GetIdTable(), "30%", "70%");			
