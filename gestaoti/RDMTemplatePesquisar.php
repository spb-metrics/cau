...
$pagina->SettituloCabecalho("Pesquisa de Templates de RDMs"); // Indica o título do cabeçalho da página
...
// Itens das abas
$aItemAba = Array( array("RDMTemplatePesquisar.php", "tabact", "Pesquisa"),
...
if($flag == "2"){
	...
	$ATIVIDADE_RB_RDM_TEMPLATE = new atividade_rdm_template();
	$ATIVIDADE_RB_RDM_TEMPLATE->deleteByRDM($seq_rdm_template);
	 
	$ATIVIDADE_RB_RDM_TEMPLATE = new atividade_rb_rdm_template();
	$ATIVIDADE_RB_RDM_TEMPLATE->deleteByRDM($seq_rdm_template);
	
	$banco = new rdm_template();
	$banco->delete($seq_rdm_template);
...
print $pagina->CampoHidden("seq_rdm_template", "");
...
// Inicio do grid de resultados
	...
	if($RDM_TEMPLATE->database->rows > 0){
		...
		while ($row = pg_fetch_array($RDM_TEMPLATE->database->result)){
			...
			$valor = $pagina->BotaoAlteraGridPesquisa("RDM_Template_Alteracao.php?seq_rdm_template=".$row["seq_rdm_template"]."");
			$valor .= $pagina->BotaoExcluiGridPesquisa("seq_rdm_template", $row["seq_rdm_template"]);
