...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Cadastro de Perfil de Acesso"); // Indica o título do cabeçalho da página
	// Itens das abas
	...
	$pagina->LinhaCampoFormulario("Nome:", "right", "S", $pagina->CampoTexto("v_NOM_PERFIL_ACESSO", "S", "Nome", "60", "60", ""), "left"); 
	...
	$banco->insert();
	// Código inserido: $banco->SEQ_PERFIL_ACESSO
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Perfil_acessoPesquisa.php?v_SEQ_PERFIL_ACESSO=$banco->SEQ_PERFIL_ACESSO");
	}
}
?>
