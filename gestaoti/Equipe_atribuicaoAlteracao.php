...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Alterção de Atribuição de Equipes de TI"); // Indica o título do cabeçalho da página
	// Itens das abas
	$aItemAba = Array( array("Equipe_atribuicaoPesquisa.php", "", "Pesquisa"),
						array("Equipe_atribuicaoCadastro.php", "", "Adicionar"),
		 			    array("Equipe_atribuicaoAlteracao.php", "tabact", "Alterar") );
	$pagina->SetaItemAba($aItemAba);
	// Pesquisar
	$banco->select($v_SEQ_EQUIPE_ATRIBUICAO);
	...
	print $pagina->CampoHidden("v_SEQ_EQUIPE_ATRIBUICAO", $v_SEQ_EQUIPE_ATRIBUICAO);
	...
	$pagina->LinhaCampoFormulario("Descrição:", "right", "S", $pagina->CampoTexto("v_DSC_EQUIPE_ATRIBUICAO", "S", "Descrição", "60", "60", "$banco->DSC_EQUIPE_ATRIBUICAO"), "left");
	...
	$banco->update($v_SEQ_EQUIPE_ATRIBUICAO);
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Equipe_atribuicaoPesquisa.php?v_SEQ_EQUIPE_ATRIBUICAO=$v_SEQ_EQUIPE_ATRIBUICAO");
	}
}
?>
