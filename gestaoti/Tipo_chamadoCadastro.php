...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Cadastro de Classe de Chamados"); // Indica o título do cabeçalho da página
	// Itens das abas
	$aItemAba = Array( array("Tipo_chamadoPesquisa.php", "", "Pesquisa"),
				  array("Tipo_chamadoCadastro.php", "tabact", "Adicionar"));
	...
	$pagina->LinhaCampoFormulario("Descrição:", "right", "S", $pagina->CampoTexto("v_DSC_TIPO_CHAMADO", "S", "Descrição", "60", "60", ""), "left");
	...
	$v_SEQ_CENTRAL_ATENDIMENTO = $_SESSION["SEQ_CENTRAL_ATENDIMENTO"];
	...
	$pagina->LinhaCampoFormulario("Utilizado no SLA?", "right", "S", $pagina->CampoSelect("v_UTILIZADO_SLA", "S", "Indicador de utilização no SLA", "S", $pagina->comboSimNao()), "left");
	...
	$banco->insert();
	// Código inserido: $banco->SEQ_TIPO_CHAMADO
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Tipo_chamadoPesquisa.php?v_SEQ_TIPO_CHAMADO=$banco->SEQ_TIPO_CHAMADO");
	}
}
?>
