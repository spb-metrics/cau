...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Cadastro de Marca Hardware"); // Indica o título do cabeçalho da página
	// Itens das abas
	...
	$pagina->LinhaCampoFormulario("Nome:", "right", "S", $pagina->CampoTexto("v_NOM_MARCA_HARDWARE", "S", "Nome", "60", "60", ""), "left"); 
	...
	$banco->insert();
	// Código inserido: $banco->SEQ_MARCA_HARDWARE
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Marca_hardwarePesquisa.php?v_SEQ_MARCA_HARDWARE=$banco->SEQ_MARCA_HARDWARE");
	}
}
?>
