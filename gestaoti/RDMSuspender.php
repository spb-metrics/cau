...
if($atividadeRDM->getSITUACAO()==$atividadeRDM->SUSPENSA){			
	$pagina->redirectTo("RDMExecutar.php?vMsgErro=Atividade já está suspensa!&v_SEQ_RDM=".$v_SEQ_RDM);
	//$pagina->redirectTo("RDMExecutar.php?v_SEQ_RDM=".$v_SEQ_RDM);
} else if($v_ACAO == "SUSPENDER"){
		$util = new util();
	...
	$pagina->SettituloCabecalho("Suspender Execução da RDM"); // Indica o título do cabeçalho da página
	...
	if($v_SEQ_RDM != "" && $v_SEQ_ID_ATM != ""  ){
		...
		$pagina->CampoButton("if(fValidaForm()){if(confirm('Deseja Suspender a RDM?')){document.form.v_ACAO.value='SUSPENDER';return true;}else{return false;}}else{return false;}", " Suspender ")
			 ."&nbsp;".	 
	...
	print $pagina->CampoHidden("v_ACAO_ATM", "SUSPENDER_ATM");
