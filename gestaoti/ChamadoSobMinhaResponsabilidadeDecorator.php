...
// Setar variáveis
	$chamado->setSEQ_SITUACAO_CHAMADO($situacao_chamado->CODS_EM_ANDAMENTO);
	$chamado->setTXT_CHAMADO($v_TXT_CHAMADO);
	$chamado->setDTH_ABERTURA($v_DTH_ABERTURA);
	$chamado->setDTH_ABERTURA_FINAL($v_DTH_ABERTURA_FINAL);
	$chamado->setSEQ_EQUIPE_TI($_SESSION["SEQ_EQUIPE_TI"]);
	$chamado->setNUM_MATRICULA_EXECUTOR($_SESSION["NUM_MATRICULA_RECURSO"]);
	...
	$header[] = array($pagina->parametro->GetValorParametro("LABEL_TIPO_CHAMADO"), "6%");
	...
	if($banco->database->rows > 0){
		...
		$DADOS->titulo = "Chamados Sob minha responsabilidade";
