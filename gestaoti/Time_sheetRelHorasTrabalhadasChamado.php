...
$pagina->SettituloCabecalho("Relatório de Horas Trabalhadas por Chamado"); // Indica o título do cabeçalho da página
...
if($flag == ""){
	...
	<script language="javascript">
		...
		function fValidaFormLocal(){
			...
			document.form.action = 'Time_sheetRelHorasTrabalhadasChamado.php';
		...
		function fValidaFormLocal1(){
			if(fValidaFormLocal()){
				document.form.action = 'Time_sheetRelHorasTrabalhadasChamadoPDF.php';
			...
			if($recurso_ti->database->rows == 0){
				...
				while ($row1 = pg_fetch_array($recurso_ti->database->result)){
					...
					if($time_sheet->database->rows == 0){
						...
						$tabela = array();
						$header = array();
						$header[] = array("Chamado", "center", "23%", "header");
						...
						while ($row2 = pg_fetch_array($time_sheet->database->result)){
							...
							$header = array();
							$header[] = array("<a href=\"Time_sheetPesquisa.php?flag=1&v_COD_DEPENDENCIA=$v_COD_DEPENDENCIA&v_SEQ_EQUIPE_TI=".$row["SEQ_EQUIPE_TI"]."&v_NUM_MATRICULA_RECURSO=".$row1["NUM_MATRICULA_RECURSO"]."&v_SEQ_CHAMADO=".$row2["SEQ_CHAMADO"]."\">".$row2["SEQ_CHAMADO"]."</a>", "center", "", "");
