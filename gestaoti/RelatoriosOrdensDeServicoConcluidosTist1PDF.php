...
$banco->selectParam("SEQ_CHAMADO, DTH_ENCERRAMENTO_EFETIVO");
	...
	while ($row = oci_fetch_array($banco->database->result, OCI_BOTH)){	
		...
		if($row['DTH_ENCERRAMENTO_EFETIVO'] != ""){
			...
			if($row['SEQ_CHAMADO'] != $seqChamado ){
				...
				$banco2->selectParam("SEQ_CHAMADO, DTH_ENCERRAMENTO_EFETIVO");
...
$pdf->Ln(2);
$pdf->Cell(300, 5, "Ordens de Serviço por Serviço : TIST-1", 0, 0, "C", 1);
$pdf->SetFont('arial');
$pdf->Ln(6);
...
$pdf->SetFillColor(300,300,300);
$pdf->SetFont('arial', '', 9);	
$pdf->Cell(100, 8, "Aprovação/Validação: TC 133-PS/2008/0001 - INFRAERO/Sede ", 1, 1, "C", 1);
...
$pdf->Cell(138, 3, "Suporte / AVANSYS", 0, 0, "C", 1);
