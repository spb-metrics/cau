...
if($v_SEQ_CHAMADO != ""){
	...
	function Inicio_Efetivo($v_DTH_INICIO_EFETIVO, $v_DTH_INICIO_PREVISAO){
		...
		if($v_DTH_INICIO_EFETIVO != ""){
			...
			if($pagina->dateDiffHour($v_DTH_INICIO_PREVISAO) < 0){ // Chamado em atraso
				...
				return "<font color=green>Chamado não iniciado - O tempo estimado para o início do atendimento &eacute; de <b>$vTempoRestante</b></font>";
			}
		}
	}
	...
	if($banco->SEQ_ITEM_CONFIGURACAO != ""){
		...
		$pagina->LinhaCampoFormulario("Sistema de Informação:", "right", "N", $item_configuracao->NOM_ITEM_CONFIGURACAO, "left", "id=".$pagina->GetIdTable());
	}
	...
	if($banco->TXT_RESOLUCAO != ""){
		$pagina->LinhaCampoFormulario("Informações sobre a solução:", "right", "N", str_replace(chr(13), "<br>",$banco->TXT_RESOLUCAO), "left", "id=".$pagina->GetIdTable());
	}
	...
	// AVALIAÇOES DO CLIENTE
	if($banco->SEQ_AVALIACAO_ATENDIMENTO != ""){
		$pagina->LinhaColspan("center", "Avaliação do cliente sobre o atendimento recebido", 2, "header");

		$pagina->LinhaCampoFormulario("Solicitação Atendida?", "right", "N", $banco->FLG_SOLICITACAO_ATENDIDA=="S"?"Sim":"Não", "left", "id=".$pagina->GetIdTable(), "30%");
		...
		$pagina->LinhaCampoFormulario("Satisfação com a solução apresentada:", "right", "N", $avaliacao_atendimento->NOM_AVALIACAO_ATENDIMENTO, "left", "id=".$pagina->GetIdTable());

		if($banco->SEQ_AVALIACAO_CONHECIMENTO_TECNICO != ""){
			$avaliacao_atendimento->select($banco->SEQ_AVALIACAO_CONHECIMENTO_TECNICO);
			$pagina->LinhaCampoFormulario("Satisfação com o conhecimento técnico do prestador de serviço:", "right", "N", $avaliacao_atendimento->NOM_AVALIACAO_ATENDIMENTO, "left", "id=".$pagina->GetIdTable());
		}

		if($banco->SEQ_AVALIACAO_POSTURA != ""){
			$avaliacao_atendimento->select($banco->SEQ_AVALIACAO_POSTURA);
			$pagina->LinhaCampoFormulario("Satisfação com a postura e cordialidade do prestador de serviço:", "right", "N", $avaliacao_atendimento->NOM_AVALIACAO_ATENDIMENTO, "left", "id=".$pagina->GetIdTable());
		}

		if($banco->SEQ_AVALIACAO_TEMPO_ESPERA != ""){
			$avaliacao_atendimento->select($banco->SEQ_AVALIACAO_TEMPO_ESPERA);
			$pagina->LinhaCampoFormulario("Satisfação com o tempo de espera para atendimento:", "right", "N", $avaliacao_atendimento->NOM_AVALIACAO_ATENDIMENTO, "left", "id=".$pagina->GetIdTable());
		}

		if($banco->SEQ_AVALIACAO_TEMPO_SOLUCAO != ""){
			$avaliacao_atendimento->select($banco->SEQ_AVALIACAO_TEMPO_SOLUCAO);
			$pagina->LinhaCampoFormulario("Satisfação com o tempo de solução:", "right", "N", $avaliacao_atendimento->NOM_AVALIACAO_ATENDIMENTO, "left", "id=".$pagina->GetIdTable());
		}

		$pagina->LinhaCampoFormulario("Observação do cliente sobre a avaliação:", "right", "N", $banco->TXT_AVALIACAO, "left", "id=".$pagina->GetIdTable());
	}
	...
	<script language="javascript">
		...
		function retorno_ControlarAtividadeTimeSheet(val) {
			if(val != ""){
				...
				// Recarregar iframe do time sheet e do histórico
...
<?
	...
	if($banco->DTH_TRIAGEM_EFETIVA != ""){
		...
		$header = array();
		$header[] = array("Previsão de atendimento 1º nível:", "center", "", "label");
		...
		// Triagem Efetiva
		$header = array();
		$header[] = array("Conclusão do atendimento 1º nível:", "center", "", "label");
	...
	if($banco->DTH_INICIO_EFETIVO != ""){
		...
		$header[] = array("Ainda não iniciado", "left", "", "campo");
	}
	$tabela[] = $header;
