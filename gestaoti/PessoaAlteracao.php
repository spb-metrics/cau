...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Alteração de Colaborador da Organização"); // Indica o título do cabeçalho da página
	// Itens das abas
	...
	if($banco->NUM_MATRICULA_RECURSO == ""){
            ...
            if($banco->database->error != ""){
                    $pagina->mensagem("Cadastro não alterado. O seguinte erro ocorreu:<br> $banco->error");
            }else{
                // Incluir o perfil de acesso
                $pagina->redirectTo("PessoaPesquisa.php?flag=1&v_SEQ_PESSOA=$v_SEQ_PESSOA");
            }
		
	}else{
		$pagina->mensagem("Cadastro não alterado. E-mail já cadastrado.");
	}
