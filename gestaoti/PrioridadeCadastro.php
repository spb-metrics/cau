...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Cadastro de Prioridade"); // Indica o título do cabeçalho da página
	// Itens das abas
	...
	$pagina->LinhaCampoFormulario("Nome:", "right", "S", $pagina->CampoTexto("v_NOM_PRIORIDADE", "S", "Nome", "60", "60", ""), "left");
	...
	$banco->insert();
	// Código inserido: $banco->SEQ_PRIORIDADE
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("PRIORIDADEPesquisa.php?v_SEQ_PRIORIDADE=$banco->SEQ_PRIORIDADE");
	}
}
?>
