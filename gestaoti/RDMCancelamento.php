...
// =======================================================================================================================
	if($flag == "1"){	 
		...
		if($v_ACAO=="CANCELAR"){		 
			$situacaoRDM->setSITUACAO($situacaoRDM->CANCELADA);
		}else {
			$situacaoRDM->setSITUACAO($situacaoRDM->CANCELADA);
		}
		...
		$rdm_email->sendEmailRDMCancelada($v_OBSERVACAO);	
		
		$pagina->redirectTo("RDMDetalhe.php?v_SEQ_RDM=".$v_SEQ_RDM);
	}
	
	$pagina->SettituloCabecalho("Cancelar  RDM"); // Indica o título do cabeçalho da página
	...
	if(($RDM->NUM_MATRICULA_SOLICITANTE == $_SESSION["NUM_MATRICULA_RECURSO"])&& 
		($RDM->getSITUACAO_ATUAL()==$situacaoRDM->CRIADA)){	
		$aItemAba[] = array("#" , "tabact", "Cancelar");
	...
	$pagina->CampoButton("if(fValidaForm()){if(confirm('Deseja cancelar a RDM?')){document.form.v_ACAO.value='CANCELAR';return true;}else{return false;}}else{return false;}", " Cancelar ")	 
