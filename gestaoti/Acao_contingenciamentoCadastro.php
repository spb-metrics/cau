...
if($flag == ""){
	...
	$pagina->SettituloCabecalho("Cadastro de Ação de Contingencia"); // Indica o título do cabeçalho da página
	// Itens das abas
	...
	$pagina->LinhaCampoFormulario("Descrição:", "right", "S", $pagina->CampoTexto("v_NOM_ACAO_CONTINGENCIAMENTO", "S", "Nome", "60", "60", ""), "left");
	...
	$banco->insert();
	// Código inserido: $banco->SEQ_ACAO_CONTINGENCIAMENTO
	if($banco->error != ""){
		$pagina->mensagem("Registro não incluído. O seguinte erro ocorreu:<br> $banco->error");
	}else{
		$pagina->redirectTo("Acao_contingenciamentoPesquisa.php?v_SEQ_ACAO_CONTINGENCIAMENTO=$banco->SEQ_ACAO_CONTINGENCIAMENTO");
	}
}
?>
